...
def clean_comments(file_path, output_file_path):
    """
    根据评论长度清理评论数据。
    评论将被过滤，只保留长度在3到40个字符之间的评论。

    Args:
        file_path (str): 输入CSV文件的路径。
        output_file_path (str): 清理后的数据将被保存的输出CSV文件路径。
    """
    ...
    try:
        ...
        # Filter comments by length (between 3 and 40 characters)
        df_filtered = df[df["reply"].str.len().between(3, 40)]
...
if __name__ == "__main__":
    # 原始数据文件（两列：文本+标签）
    init_file_path = "/home/lc/lc/Test/data/initial_data/e_comments.csv"

    # 由原始数据文件经过长度清洗得到 并作为之后的主要数据处理文件
    comments_file_path = "/home/lc/lc/Test/data/gen_data/comments_cleaned.csv"
    # 停用词词典的路径
    stopwords_txt_path = "/home/lc/lc/Test/data/initial_data/stopwords.txt"
    # 情感词典的路径
    sen_dict_path = "/home/lc/lc/Test/data/initial_data/BosonNLP_sentiment_score.txt"
    # 否定词词典的路径
    neg_dict_path = "/home/lc/lc/Test/data/initial_data/NOT.txt"
    # 副词词典的路径
    adv_dict_path = "/home/lc/lc/Test/data/initial_data/adv_chinese.txt"
    # Emoji数据的路径
    emoji_data_path = "/home/lc/lc/Test/data/initial_data/emoji_data.csv"

    # 清理评论数据，仅保留长度在特定范围内的评论
    clean_comments(init_file_path, comments_file_path)
